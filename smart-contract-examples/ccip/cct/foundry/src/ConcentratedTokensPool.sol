...
abstract contract ConcentratedTokensPool is IPoolV1, OwnerIsCreator {
    ...
    address internal s_tokensManager;
    ...
    function isSupportedToken(address token) public view virtual returns (bool) {
        if (msg.sender != s_tokensManager) return true;
    ...
    function setTokensManager(address tokensManager) external onlyOwner {
        s_tokensManager = tokensManager;
    }
    ...
    function _validateReleaseOrMint(Pool.ReleaseOrMintInV1 memory releaseOrMintIn) internal {
        ...
        _consumeInboundRateLimit(
            releaseOrMintIn.remoteChainSelector, releaseOrMintIn.amount, releaseOrMintIn.localToken
        );
    }
    ...
    function _updateRemoteChainConfig(ChainUpdate memory update) internal {
