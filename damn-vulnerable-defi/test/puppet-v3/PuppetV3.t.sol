...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        ...
        bytes memory path = abi.encode(address(token), address(weth));
        ...
        console.log(
            "pool token balance",
            token.balanceOf(address(uniswapPool))
        );
        console.log("pool weth balance", weth.balanceOf(address(uniswapPool)));
        console.log("pool native balance", address(uniswapPool).balance);
